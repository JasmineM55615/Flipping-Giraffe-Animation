...
var glink1, glink2, glink3, glink4, glink5, glink6, glink7, glink8, glink9, glink10, glink11, glink12
var glinkFrame1, glinkFrame2, glinkFrame3, glinkFrame4, glinkFrame5, glinkFrame6, glinkFrame7, 
glinkFrame8, glinkFrame9, glinkFrame10, glinkFrame11, glinkFrame12;
var sphere;    
var mybox;     
var meshes = {};  
...
function initMotions() {
    ...
    myboxMotion.addKeyFrame(new Keyframe('rest pose',2.0, [-3, 10, 0, 0])); //CHANGED: y coordinate
    myboxMotion.addKeyFrame(new Keyframe('rest pose',3.0, [0, 10, 0, 0]));  //CHANGED: y coordinate
    ...
    handMotion.addKeyFrame(new Keyframe('straight',         0.0, [, 3,    0, 0, 0, 0, 0]));
    handMotion.addKeyFrame(new Keyframe('link1 comes down', 0.5, [2, 3,    90, 0, 0, 0, 0])); //CHANGED (added this lines)
    handMotion.addKeyFrame(new Keyframe('right finger curl',1.0, [2, 3,    0, +90, -180, 0,0])); //CHANGED -90 -> -180
...
function handSetMatrices(avars) {
    ...
    link1.matrix.multiply(M.makeScale(6,1,3)); //CHANGED (4, 1, 1) -> (6, 1, 3)
    ...
    link2.matrix.multiply(M.makeTranslation(3,0,0));   //CHANGED 2 -> 3
    ...
    link3.matrix.multiply(M.makeTranslation(2.5,0,0)); //CHANGED 2 -> 2.5
    ...
    link4.matrix.multiply(M.makeTranslation(3,0,0));   //CHANGED 2 -> 3
    ...
    link5.matrix.multiply(M.makeTranslation(2.5,0,0));   //CHANGED 2 -> 2.5
...
function giraffeSetMatrices(avars) {
  ...
  var Mg = new THREE.Matrix4();
    ...
    // link1
    glinkFrame1.matrix.identity(); 
    glinkFrame1.matrix.multiply(M.makeTranslation(xPosition + 4.0, yPosition, 0));   
    glinkFrame1.matrix.multiply(M.makeRotationZ(theta1));    
      // Frame 1 has been established, now setup the extra transformations for the scaled box geometry
    glink1.matrix.copy(glinkFrame1.matrix);
    glink1.matrix.multiply(Mg.makeTranslation(0,0,0));   
    glink1.matrix.multiply(Mg.makeScale(6,1,3)); //CHANGED (4, 1, 1) -> (6, 1, 3)

    // link2
    glinkFrame2.matrix.copy(glinkFrame1.matrix);      // start with parent frame
    glinkFrame2.matrix.multiply(M.makeTranslation(4,0,1));
    glinkFrame2.matrix.multiply(M.makeRotationZ(theta2));    
      // Frame 2 has been established, now setup the extra transformations for the scaled box geometry
    glink2.matrix.copy(glinkFrame2.matrix);
    glink2.matrix.multiply(M.makeTranslation(3,0,0));   //CHANGED 2 -> 3
    glink2.matrix.multiply(M.makeScale(4,1,1));   


    glink1.updateMatrixWorld();
    glink2.updateMatrixWorld();

    glinkFrame1.updateMatrixWorld();
    glinkFrame2.updateMatrixWorld();
...
// ADDED geometry for my giraffe

function initGiraffe() {
  var giraffeMaterial = new THREE.MeshLambertMaterial( {color: 0xFFC0CB} );
  var boxGeometry = new THREE.BoxGeometry( 2, 2, 2 );    // width, height, depth

  glink1 = new THREE.Mesh( boxGeometry, giraffeMaterial );  scene.add( glink1 );
  glinkFrame1   = new THREE.AxesHelper(1) ;   scene.add(glinkFrame1);
  glink2 = new THREE.Mesh( boxGeometry, giraffeMaterial );  scene.add( glink2 );
  glinkFrame2   = new THREE.AxesHelper(1) ;   scene.add(linkFrame2);

  glink1.matrixAutoUpdate = false; 
  glink2.matrixAutoUpdate = false; 

  glinkFrame1.matrixAutoUpdate = false;
  glinkFrame2.matrixAutoUpdate = false; 
